...
async def test_check_bot_status_in_meeting():
    ...
    with patch(
        "src.services.recall_service.RecallService.is_bot_in_meeting"
    ) as mock_check:
...
async def test_recall_service_is_bot_in_meeting():
    ...
    with patch.object(
        RecallService, "retrieve_bot_data", new=AsyncMock(return_value=mock_bot_data)
    ):
...
async def test_recall_service_bot_no_status_changes():
    ...
    mock_bot_data = {"id": bot_id, "status_changes": []}
