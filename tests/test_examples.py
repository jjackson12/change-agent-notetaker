...
class ExampleMockedTests:
    ...
    async def test_example_bot_joins_meeting(self, mock_factory):
        ...
        # Step 1: Create bot
        create_response = mock_factory.create_bot_response(
            bot_id="example_bot_456"
        )
    ...
    async def test_example_webhook_processing(self, mock_factory):
        ...
        with MockRecallAPI(bot_data=complete_bot_data), \
...
class ExampleErrorHandlingTests:
    ...
    async def test_example_bot_error_event(self, mock_factory):
        ...
        # Create meeting
        with patch('src.services.recall_service.RecallService.create_bot',
                   new=AsyncMock(return_value=mock_factory.create_bot_response(bot_id="error_bot"))):
            response = client.post("/meeting_bots/send-bot", json={
