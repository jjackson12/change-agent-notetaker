...
def mock_recall_bot_response():
    ...
    return {
        ...
        "status_changes": [{"code": "ready", "created_at": "2023-10-01T10:00:00Z"}],
    }
...
def mock_bot_done_response():
    ...
    return {
        ...
        "meeting_metadata": {"title": "Team Standup"},
        "recordings": [
            {
                ...
                "media_shortcuts": {
                    "transcript": {
                        "data": {"download_url": "https://recall.ai/transcript/abc123"}
                    },
                    "participant_events": {
                        "data": {
...
def mock_transcript_data():
    ...
    return [
        {
            "participant": {"name": "Alice", "id": "participant_1"},
            "words": [
                {
                    "text": "Hello",
        ...
        {
            "participant": {"name": "Bob", "id": "participant_2"},
            "words": [
                {
                    "text": "Hi",
...
def mock_summary_response():
    ...
    return {
        ...
        "action_items": ["Alice: Review Bob's PR", "Bob: Complete dashboard by EOD"],
        "decisions": ["Deploy to staging tomorrow"],
    }
...
class TestMeetingWorkflowMocked:
    ...
    async def test_complete_meeting_workflow(
        ...
        with patch(
        ...
        ), patch("httpx.AsyncClient.get") as mock_http_get, patch(
            ...
            assert (
                webhook_response["message"] == "Webhook received and processing started"
            )
    ...
    async def test_bot_error_workflow(self, mock_recall_bot_response):
        ...
        webhook_payload = {"event": "bot.error", "data": {"bot": {"id": bot_id}}}
...
class TestMeetingWorkflowHelpers:
    ...
    @staticmethod
    async def wait_for_meeting_status(
        meeting_id: int, expected_status: str, timeout: int = 10
    ):
        """
        ...
        while True:
            ...
            if asyncio.get_event_loop().time() - start_time > timeout:
                raise TimeoutError(
                    f"Meeting did not reach status {expected_status} within {timeout}s"
                )
