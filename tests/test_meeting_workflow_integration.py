...
class TestRealMeetingWorkflow:
    ...
    async def test_full_real_meeting_workflow(self):
        ...
        response = client.post(
            "/meeting_bots/send-bot", json={"meeting_url": meeting_url, "user_id": 1}
        )
        ...
        meeting_completed = await self._wait_for_meeting_completion(
            meeting_id, timeout=900
        )
        ...
        final_meeting = await self._wait_for_status(
            meeting_id, MeetingStatus.DONE, timeout=180
        )
    ...
    async def _wait_for_bot_in_meeting(
        self, meeting_id: int, timeout: int = 120
    ) -> bool:
        """
    ...
    async def _wait_for_meeting_completion(
        self, meeting_id: int, timeout: int = 900
    ) -> bool:
        """
    ...
    async def _wait_for_status(
        self, meeting_id: int, expected_status: MeetingStatus, timeout: int = 180
    ):
        """
        ...
        while True:
            ...
            remaining = timeout - elapsed
            print(
                f"  Waiting for {expected_status.value}... ({remaining:.0f}s remaining)"
            )
            await asyncio.sleep(poll_interval)
    ...
    def _verify_meeting_data(self, meeting_data: dict):
        ...
        assert (
            meeting_data["status"] == "done"
        ), f"Expected status 'done', got '{meeting_data['status']}'"
        ...
        assert isinstance(
            meeting_data["participants"], list
