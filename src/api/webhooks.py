            ...
            try:
                ...
                if processed_data["transcript"]:
                    try:
                        ...
                        logger.info(f"AI summary generated successfully for botId: {bot_id}")
                    except Exception as summary_error:
                        logger.error(f"Failed to generate AI summary for {bot_id}: {summary_error}")
                ...
                if data.get("meeting_metadata"):
                    meeting.title = data["meeting_metadata"].get("title", "Completed Meeting")
                    meeting.participants = data["meeting_metadata"].get("participants", [])
                meeting.status = MeetingStatus.DONE
                db.commit()
